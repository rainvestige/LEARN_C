...
int getop(char s[]) {
  int i, c;
  static char buf_char = ' ';
  // skip whitespaces and tabs
  c = (buf_char == ' ' ? getchar() : buf_char);
  buf_char = ' ';
  while ((s[0] = c) == ' ' || c == '\t') {
    c = (buf_char == ' ' ? getchar() : buf_char);
    buf_char = ' ';
  ...
  if (!isalpha(c) && !isdigit(c) && c != '.' && c != '-')
    return c;
  if (c == '-' && !isdigit(s[1] = (buf_char == ' ' ? getchar() : buf_char))) {
    // minus operator `-`
    buf_char = s[1];  // otherwise the `\n` right after `-` will be discarded
    return c;
  } else if (c == '-')
    // push back on input
    buf_char = s[1];
  ...
  if (isalpha(c)) {
    while(isalpha(s[++i] = c = (buf_char == ' ' ? getchar() : buf_char)))
      ;
    buf_char = ' ';
    s[i] = '\0';
    if (c != EOF) buf_char = c;
  ...
  if (isdigit(c) || c == '-')  // collect integer part
    ...
    while (isdigit(s[++i] = c = (buf_char == ' ' ? getchar() : buf_char)))
      ;
  buf_char = ' ';
